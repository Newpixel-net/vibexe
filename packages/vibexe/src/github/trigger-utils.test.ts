import type { WebhookEvent } from "@vibexe-ai/github-tool";
import {
	type Output,
	OutputId,
	type Trigger,
...
vi.mock("@vibexe-ai/github-tool", async () => {
	const actual = await vi.importActual<
		typeof import("@vibexe-ai/github-tool")
	>("@vibexe-ai/github-tool");
	return {
		...actual,
...
function createTrigger(eventId: GitHubEventId): Trigger {
	const event =
		...
		eventId === "github.discussion_comment.created"
			? { id: eventId, conditions: { callsign: "vibexe" } }
			: { id: eventId };
	return {
