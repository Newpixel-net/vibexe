import type { DataStoreProvider } from "@vibexe-ai/data-store-registry";
import { type VibexeLogger, noopLogger } from "@vibexe-ai/logger";
import {
	type DataStore,
	type DataStoreId,
...
import { bindVibexeFunction } from "./utils/create-vibexe-function";
import {
	copyWorkspace,
...
export function Vibexe(config: VibexeConfig) {
	const context: VibexeContext = {
		...config,
	...
	return {
		...
		getContext: (): VibexeContext => context,
		...
		saveApp: bindVibexeFunction(saveApp, context),
		deleteApp: bindVibexeFunction(deleteApp, context),
		getApp: bindVibexeFunction(getApp, context),
...
export type Vibexe = ReturnType<typeof Vibexe>;
