...
import { createHttpHandler } from "./next-vibexe-engine";

// Mock the module dependencies
vi.mock("@vibexe-ai/http", () => {
...
describe("createHttpHandler", () => {
	...
	} as unknown as Vibexe;
	const basePath = "/api/vibexe";
	...
	it("should handle generated image multi-segment path", async () => {
		...
		expect(mockVibexe.getGeneratedImage).toHaveBeenCalledWith(
			generationId,
			filename,
		);
