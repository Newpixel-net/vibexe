...
var SKY_FRAGMENT = [
  ...
  "",
  "    // Multi-tier: dim (1.5% of cells), medium (0.7%), bright (0.3%)",
  "    float tier1 = smoothstep(0.985, 0.992, sh) * 0.4;",
  "    float tier2 = smoothstep(0.992, 0.997, sh) * 0.7;",
  "    float tier3 = smoothstep(0.997, 1.0, sh);",
  "    float bright = max(max(tier1, tier2), tier3);",
  "",
  "    // Soft circular glow — radius scales with brightness",
  "    float radius = 0.25 + bright * 0.2;",
  "    float glow = exp(-dist * dist / (radius * radius));",
  "",
  "    // 4-point diffraction spikes on bright stars",
  "    float spikes = 0.0;",
  "    if (sh > 0.993) {",
  "      float sLen = 0.15 + (sh - 0.993) * 20.0;",
  "      float sx = exp(-abs(cellUV.x) / sLen * 3.0) * exp(-abs(cellUV.y) * 8.0);",
  "      float sy = exp(-abs(cellUV.y) / sLen * 3.0) * exp(-abs(cellUV.x) * 8.0);",
  "      spikes = (sx + sy) * 0.4;",
  "    }",
  "",
  "    // Color temperature variation (blue-white to warm-white)",
  "    vec3 starCol = mix(vec3(0.75, 0.85, 1.0), vec3(1.0, 0.92, 0.8), fract(sh * 17.3));",
  "",
  "    float twinkle = sin(sh * 6283.0 + uTime * 1.5) * 0.2 + 0.8;",
  "    float fade = smoothstep(0.02, 0.12, dir.y);",
  "    sky += starCol * (bright * glow + spikes) * twinkle * starV * fade * 1.2;",
  "  }",
  "",
  ...
  "  }",
  "",
  "  // Procedural clouds — multi-scale for billowy shapes",
  "  if (uCloudCover > 0.01 && dir.y > 0.01) {",
  "    vec2 cUV = dir.xz / (dir.y + 0.08) * uCloudScale + uCloudOff;",
  "    // Large base shapes (low freq) + fine edge detail (high freq)",
  "    float cnBase = cfbm(cUV * 0.4);",
  "    float cnDetail = cfbm(cUV * 1.2 + vec2(3.7, 1.3));",
  "    float cn = cnBase * 0.7 + cnDetail * 0.3;",
  "    float thresh = 1.0 - uCloudCover;",
  "    // Sharper edge transition for distinct cloud boundaries",
  "    float cMask = smoothstep(thresh, thresh + 0.08, cn) * uCloudDens;",
  "    // Internal density variation for depth/volume feel",
  "    float cDepth = smoothstep(thresh, thresh + 0.25, cn);",
  ...
  "    // Brighter highlights, darker cores for 3D volume feel",
  "    float cLight = mix(0.15, 1.0, daylight) * mix(0.65, 1.35, sunFace);",
  "    vec3 cCol = vec3(cLight * uCloudBright * mix(0.7, 1.0, cDepth));",
  "    float nightTint = 1.0 - daylight;",
  "    cCol = mix(cCol, cCol * vec3(0.35, 0.40, 0.60), nightTint);",
  "    // Silver lining on cloud edges facing the sun",
  "    float edgeGlow = smoothstep(thresh + 0.06, thresh + 0.01, cn) * 0.45 * daylight * sunFace;",
  "    cCol += vec3(1.0, 0.95, 0.80) * edgeGlow;",
