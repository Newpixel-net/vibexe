...
export function buildAssetReferencePrompt(): string {
	const lines: string[] = [
		...
		"",
		"Drawing helpers use a **3-tier fallback chain** for maximum visual quality:",
		"1. **Sprite library** — Pre-made 2D sprites and 3D-rendered animated sprite sheets (best quality)",
		"2. **Canvas 2D** — Programmatic drawing with shadowBlur, radial gradients, specular highlights",
		"3. **PIXI.Graphics** — Shape-based fallback (always works)",
		"",
		"The fallback is AUTOMATIC — just call the helper functions normally. If sprites are available they're used first.",
		"",
		"### Sprite Library (import { _loadSpriteLib } from '../utils/media-stock')",
		"",
		"**MUST call `await _loadSpriteLib(THEME)` in enter() BEFORE creating any game objects.**",
		"This preloads all available sprites silently — no errors if sprites are missing.",
		"",
		"Available sprite categories:",
		"- **platforms**: grass_block, stone_block, ice_block, sand_block, dark_block",
		"- **trees**: green_tree, tree_top, tree_top_snow, dead_tree_ice, tree_trunk, tree_trunk_bottom",
		"- **bushes**: bush_large",
		"- **clouds**: cloud_large, cloud_medium, cloud_small",
		"- **collectibles**: coin_gold, gem_red, gem_blue, star, heart",
		"- **props**: crate, barrel, rock, fence, sign_post",
		"- **backgrounds**: hill_green, hill_snow, mountain_rock, bg_green, bg_castle",
		"",
		"Available character sprite sheets (AnimatedSprite with animation states):",
		"- **hero**: idle, walk, jump, attack, die",
		"- **slime**: idle, move, attack, die",
		"- **bat**: idle, fly, attack, die",
		"- **boss**: idle, attack, die",
		"- **npc**: idle, talk",
		"",
		"### Character Animation Pattern",
		"",
		"```typescript",
		"// Characters are AnimatedSprite when sprite sheets are available:",
		"var playerGfx = drawPlayerCharacter(48, PAL.player, PAL.playerLight);",
		"// Returns AnimatedSprite if sheet loaded, Canvas sprite or PIXI.Graphics otherwise",
		"",
		"// Switch animation (only works if it's an AnimatedSprite):",
		"if (playerGfx.textures && playerGfx.play) {",
		"  var sheet = _sheetCache && _sheetCache['hero'];",
		"  if (sheet && sheet.animations['walk']) {",
		"    playerGfx.textures = sheet.animations['walk'];",
		"    playerGfx.play();",
		"  }",
		"}",
		"```",
		...
		"",
		"All helpers below automatically use the best available rendering method (sprite → canvas → graphics):",
		"",
		...
		"- drawMountainRange(worldW, baseY, color, alpha, minH, maxH, spacing) — bezier curve hills",
		"- drawCloud(w, h) — puffy cloud (uses sprite if available)",
		"- drawTree(trunkH, leafR, trunkColor, leafColor) — tree (uses sprite if available)",
		"- drawPlatformBlock(w, h, mainColor, topColor) — platform with grass (sprite → canvas → graphics)",
		"- drawPlayerCharacter(size, bodyColor, lightColor) — **AnimatedSprite → canvas → graphics** with face details",
		"- drawCoinToken(radius, color, glowColor) — golden coin (sprite → canvas → graphics) with glow",
		"- drawEnemySlime(size, color, lightColor) — **AnimatedSprite → canvas → graphics** angry blob",
		"- drawHeart(size, color) — heart shape (uses sprite if available)",
		"- drawGroundStrip(worldW, groundY, floorH, color, topColor) — gradient fill ground with grass tufts",
		"- drawGemShape(radius, color) — hexagonal gem (uses sprite if available) with glow",
		...
		"- NEVER forget `await _loadSpriteLib(THEME)` in enter() — sprites won't be used",
		"",
	];
