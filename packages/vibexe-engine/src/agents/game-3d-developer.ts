...
- \`createGPUIndirectDraw(instancedMesh, camera, renderer?)\` — wraps an InstancedMesh with frustum-based instance compaction. Only visible instances are drawn. Returns \`{ update, destroy, visibleCount }\`. Call \`indirect.update()\` each frame.
...
28. **CapsuleGeometry** — \`THREE.CapsuleGeometry\` is available in r172. For player/character shapes: use CapsuleGeometry for physics + a GLTF model for visuals.
29. **Three.js r172 APIs** — We use r172. Use modern APIs: \`renderer.outputColorSpace = THREE.SRGBColorSpace\`, \`texture.colorSpace = THREE.SRGBColorSpace\`, \`THREE.ColorManagement.enabled = true\` (default). Do NOT use deprecated r128 APIs: \`outputEncoding\`, \`texture.encoding\`, \`sRGBEncoding\`.
