...
export function FlowControlPropertiesPanel({
	...
	switch (node.content.type) {
		case "if":
			return <IfPanel node={node as IfNode} onDelete={() => deleteNode(node.id)} />;
		case "switch":
			return <SwitchPanel node={node as SwitchNode} onDelete={() => deleteNode(node.id)} />;
		case "merge":
			return <MergePanel node={node as MergeNode} onDelete={() => deleteNode(node.id)} />;
		case "loop":
			return <LoopPanel node={node as LoopNode} onDelete={() => deleteNode(node.id)} />;
		case "code":
			return <CodePanel node={node as CodeNode} onDelete={() => deleteNode(node.id)} />;
		case "filter":
			return <FilterPanel node={node as FilterNode} onDelete={() => deleteNode(node.id)} />;
		case "editFields":
			return <EditFieldsPanel node={node as EditFieldsNode} onDelete={() => deleteNode(node.id)} />;
		case "sort":
			return <SortPanel node={node as SortNode} onDelete={() => deleteNode(node.id)} />;
		case "wait":
			return <WaitPanel node={node as WaitNode} onDelete={() => deleteNode(node.id)} />;
		case "errorTrigger":
			return <ErrorTriggerPanel node={node as ErrorTriggerNode} onDelete={() => deleteNode(node.id)} />;
	}
}

// ---- If Panel ----
function IfPanel({ node, onDelete }: { node: IfNode; onDelete: () => void }) {
				...
				<div className="flex flex-col gap-[16px]">
					<div>
						...
						<p className="text-[11px] text-text-muted/50">
							Evaluates conditions on input data. Routes to True or False output port.
						</p>
					</div>
					...
					{node.content.conditionGroup.conditions.map((cond, i) => (
						<ConditionRow
							...
							condition={cond}
							onChange={(updated) => {
								const conditions = [...node.content.conditionGroup.conditions];
							...
							onRemove={() => {
								const conditions = node.content.conditionGroup.conditions.filter((_, j) => j !== i);
								updateContent(node, {
									conditionGroup: {
										...node.content.conditionGroup,
										conditions,
...
function ConditionRow({
	...
	onChange: (updated: { field: string; operator: ConditionOperator; value?: string }) => void;
	onRemove: () => void;
}) {
	return (
...
// ---- Switch Panel ----
function SwitchPanel({ node, onDelete }: { node: SwitchNode; onDelete: () => void }) {
	const updateContent = useUpdateNodeDataContent();
			...
			<PropertiesPanelContent>
				<div className="flex flex-col gap-[16px]">
					<div>
						...
						<p className="text-[11px] text-text-muted/50">
							Evaluates rules in order. First matching rule activates its output port.
						</p>
					</div>
					<div className="flex items-center gap-[8px]">
						<label className="text-[12px] text-text-muted">
							<input
								...
								onChange={(e) => updateContent(node, { hasFallback: e.target.checked })}
							/>
							Enable fallback output
						</label>
					</div>
					{node.content.rules.map((rule, i) => (
						<div key={`rule-${i}`} className="rounded-[8px] border border-border-muted p-[12px] flex flex-col gap-[8px]">
							<div className="flex items-center gap-[8px]">
								<input
									type="text"
								...
								<input
									...
									onChange={(e) => {
										const rules = [...node.content.rules];
										rules[i] = { ...rule, outputPortName: e.target.value };
										updateContent(node, { rules });
									}}
								...
								<button
									...
									onClick={() => {
										const rules = node.content.rules.filter((_, j) => j !== i);
										updateContent(node, { rules });
									}}
								>
									x
...
// ---- Merge Panel ----
function MergePanel({ node, onDelete }: { node: MergeNode; onDelete: () => void }) {
	const updateContent = useUpdateNodeDataContent();
			...
			<PropertiesPanelContent>
				<div className="flex flex-col gap-[16px]">
					...
					<select
						...
						onChange={(e) => updateContent(node, { mode: e.target.value as MergeNode["content"]["mode"] })}
					>
						<option value="chooseBranch">Choose Branch (use whichever ran)</option>
...
// ---- Loop Panel ----
function LoopPanel({ node, onDelete }: { node: LoopNode; onDelete: () => void }) {
	const updateContent = useUpdateNodeDataContent();
				...
				<div className="flex flex-col gap-[16px]">
					<div>
						...
						<select
							className="rounded-[8px] border border-border-muted bg-transparent px-[12px] py-[8px] text-[13px] text-text"
							value={node.content.mode}
							onChange={(e) => updateContent(node, { mode: e.target.value as "forEach" | "nTimes" })}
						>
					...
					{node.content.mode === "nTimes" && (
						<div>
							...
							<input
								...
								onChange={(e) => updateContent(node, { nTimes: Number(e.target.value) })}
							/>
						</div>
					)}
					<div>
						...
						<input
							...
							min={1}
							max={10000}
							onChange={(e) => updateContent(node, { maxIterations: Number(e.target.value) })}
						/>
...
// ---- Code Panel ----
function CodePanel({ node, onDelete }: { node: CodeNode; onDelete: () => void }) {
	const updateContent = useUpdateNodeDataContent();
			...
			<PropertiesPanelContent>
				<div className="flex flex-col gap-[16px]">
					<div>
						...
						<p className="text-[11px] text-text-muted/50">
							Input data is available as <code>items</code> (array) and <code>data</code> (object).
...
// ---- Filter Panel ----
function FilterPanel({ node, onDelete }: { node: FilterNode; onDelete: () => void }) {
	const updateContent = useUpdateNodeDataContent();
			...
			<PropertiesPanelContent>
				<div className="flex flex-col gap-[16px]">
					<div>
						...
						<p className="text-[11px] text-text-muted/50">
							Items matching conditions go to Kept output, others to Discarded.
						</p>
					</div>
					<select
						className="rounded-[8px] border border-border-muted bg-transparent px-[12px] py-[8px] text-[13px] text-text"
					...
					{node.content.conditionGroup.conditions.map((cond, i) => (
						<ConditionRow
							...
							onChange={(updated) => {
								...
								conditions[i] = updated;
								updateContent(node, {
									conditionGroup: { ...node.content.conditionGroup, conditions },
								});
							}}
							onRemove={() => {
								const conditions = node.content.conditionGroup.conditions.filter((_, j) => j !== i);
...
// ---- Edit Fields Panel ----
function EditFieldsPanel({ node, onDelete }: { node: EditFieldsNode; onDelete: () => void }) {
	const updateContent = useUpdateNodeDataContent();
				...
				<div className="flex flex-col gap-[16px]">
					...
					<label className="text-[12px] text-text-muted">
						<input
							...
							onChange={(e) => updateContent(node, { keepOnlySet: e.target.checked })}
						/>
					...
					{node.content.operations.map((op, i) => (
						<div key={`op-${i}`} className="flex gap-[4px] items-end">
							<div className="w-[90px]">
								<label className="text-[10px] text-text-muted/70">Operation</label>
								<select
									className="w-full rounded-[6px] border border-border-muted bg-transparent px-[6px] py-[6px] text-[12px] text-text"
									value={op.operation}
									onChange={(e) => {
										const operations = [...node.content.operations];
										operations[i] = { ...op, operation: e.target.value as "set" | "remove" | "rename" };
							...
							<div className="flex-1">
								<label className="text-[10px] text-text-muted/70">Field Name</label>
								<input
									type="text"
									className="w-full rounded-[6px] border border-border-muted bg-transparent px-[8px] py-[6px] text-[12px] text-text"
									value={op.fieldName}
									onChange={(e) => {
										const operations = [...node.content.operations];
										operations[i] = { ...op, fieldName: e.target.value };
										updateContent(node, { operations });
									}}
								/>
							</div>
							{op.operation === "set" && (
								<div className="flex-1">
									...
									<input
										...
										onChange={(e) => {
											const operations = [...node.content.operations];
											operations[i] = { ...op, value: e.target.value };
											updateContent(node, { operations });
										}}
									/>
								</div>
							...
							{op.operation === "rename" && (
								<div className="flex-1">
									<label className="text-[10px] text-text-muted/70">New Name</label>
									<input
										type="text"
										className="w-full rounded-[6px] border border-border-muted bg-transparent px-[8px] py-[6px] text-[12px] text-text"
										...
										onChange={(e) => {
											const operations = [...node.content.operations];
											operations[i] = { ...op, newFieldName: e.target.value };
											updateContent(node, { operations });
										}}
									/>
								</div>
							...
							<button
								...
								onClick={() => {
									const operations = node.content.operations.filter((_, j) => j !== i);
									updateContent(node, { operations });
								}}
							>
								x
...
// ---- Sort Panel ----
function SortPanel({ node, onDelete }: { node: SortNode; onDelete: () => void }) {
	const updateContent = useUpdateNodeDataContent();
			...
			<PropertiesPanelContent>
				<div className="flex flex-col gap-[16px]">
					...
					{node.content.sortKeys.map((key, i) => (
						<div key={`sort-${i}`} className="flex gap-[4px] items-end">
							<div className="flex-1">
								<label className="text-[10px] text-text-muted/70">Field</label>
								<input
									type="text"
									...
									value={key.field}
									onChange={(e) => {
										const sortKeys = [...node.content.sortKeys];
										sortKeys[i] = { ...key, field: e.target.value };
							...
							<div className="w-[100px]">
								<label className="text-[10px] text-text-muted/70">Direction</label>
								<select
									className="w-full rounded-[6px] border border-border-muted bg-transparent px-[6px] py-[6px] text-[12px] text-text"
									value={key.direction}
									onChange={(e) => {
										const sortKeys = [...node.content.sortKeys];
										sortKeys[i] = { ...key, direction: e.target.value as "asc" | "desc" };
										updateContent(node, { sortKeys });
									}}
								>
							...
							<button
								...
								className="shrink-0 px-[6px] py-[6px] text-[12px] text-error-500"
								onClick={() => {
									const sortKeys = node.content.sortKeys.filter((_, j) => j !== i);
									updateContent(node, { sortKeys });
								}}
							>
								x
					...
					<button
						...
						onClick={() =>
							updateContent(node, {
								sortKeys: [...node.content.sortKeys, { field: "", direction: "asc" }],
							})
						}
					>
						+ Add Sort Key
...
// ---- Wait Panel ----
function WaitPanel({ node, onDelete }: { node: WaitNode; onDelete: () => void }) {
	const updateContent = useUpdateNodeDataContent();
				...
				<div className="flex flex-col gap-[16px]">
					<div>
						...
						<select
							className="rounded-[8px] border border-border-muted bg-transparent px-[12px] py-[8px] text-[13px] text-text"
							value={node.content.mode}
							onChange={(e) => updateContent(node, { mode: e.target.value as "fixedTime" | "webhook" | "approval" })}
						>
					...
					{node.content.mode === "fixedTime" && (
						<div>
							...
							<input
								...
								min={0}
								onChange={(e) => updateContent(node, { delaySeconds: Number(e.target.value) })}
							/>
						</div>
					)}
					<div>
						...
						<input
							...
							min={1}
							onChange={(e) => updateContent(node, { timeoutSeconds: Number(e.target.value) })}
						/>
					</div>
				</div>
			</PropertiesPanelContent>
...
// ---- Error Trigger Panel ----
function ErrorTriggerPanel({ node, onDelete }: { node: ErrorTriggerNode; onDelete: () => void }) {
	return (
		<PropertiesPanelRoot>
			<NodePanelHeader node={node} onDelete={onDelete} />
			<PropertiesPanelContent>
				<div className="flex flex-col gap-[16px]">
					<div>
						...
						<p className="text-[11px] text-text-muted/50">
							...
							Connect it to downstream nodes to handle errors (e.g., send notifications).
						</p>
					</div>
