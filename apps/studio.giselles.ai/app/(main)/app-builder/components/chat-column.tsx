...
const getMessagesStorageKey = (appId: string) => `giselle-builder-messages-${appId}`;
    ...
    <div className="mx-4 mb-4 p-4 rounded-lg bg-green-500/10 border border-green-500/20">
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-muted-foreground">Your app is ready for deployment.</p>
        </div>
...
export function ChatColumn({
  ...
  const { messages, setMessages, sendMessage, status, error, stop } = useChat({
    ...
    onFinish: ({ message }) => {
      ...
      const hasFileChanges = message.parts?.some((part) => {
        ...
        if (p.output?.path) {
          if (p.output.success) {
            setPhaseTimeline((prev) => markFileCompleted(prev, p.output!.path!));
          } else {
            setPhaseTimeline((prev) => markFileError(prev, p.output!.path!));
          }
        }
        return (
          p.output?.success &&
      ...
      try {
        ...
        if (stored) {
          ...
          if (Array.isArray(parsedMessages) && parsedMessages.length > 0) {
            console.log("[ChatColumn] Restoring", parsedMessages.length, "messages from localStorage");
            setMessages(parsedMessages);
          }
        }
      } catch (e) {
        console.error("[ChatColumn] Error loading messages from localStorage:", e);
      }
    }
  ...
  useEffect(() => {
    if (hasMounted && messages.length > 0) {
      localStorage.setItem(getMessagesStorageKey(appId), JSON.stringify(messages));
    }
