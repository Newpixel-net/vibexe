 ...
 *
 * Displays project stages and detailed phase timeline with file generation progress.
 * Features collapsible phases, status indicators, and auto-scroll.
 *
 * Deploy to: /opt/giselle/apps/studio.giselles.ai/app/(main)/app-builder/components/phase-timeline.tsx
 */
...
import type { PhaseTimelineItem, ProjectStage, FileType } from "../types/vibesdk";
...
const getPhaseByStatus = (
  phases: PhaseTimelineItem[],
  status: PhaseTimelineItem["status"]
...
function StatusIcon({ status, size = "md", className }: StatusIconProps) {
  ...
  switch (status) {
    case "generating":
      return (
        <Loader
          className={cn(iconClasses, "animate-spin text-primary", className)}
        />
      );
    case "validating":
      return (
        <Loader
          className={cn(iconClasses, "animate-spin text-blue-400", className)}
        />
      );
    case "completed":
      return (
        <Check className={cn(iconClasses, "text-green-500", className)} />
      );
    case "cancelled":
      return (
        <XCircle className={cn(iconClasses, "text-orange-400", className)} />
      );
    case "error":
      return (
        <AlertCircle className={cn(iconClasses, "text-red-500", className)} />
      );
    case "active":
      return (
        <Loader
...
export interface PhaseTimelineProps {
  /** High-level project stages: Setup, Plan, Build */
  projectStages: ProjectStage[];
  /** Detailed phase timeline items */
  phaseTimeline: PhaseTimelineItem[];
  /** All files in the project */
  files: FileType[];
  /** Current view: 'editor' | 'preview' | 'blueprint' */
  view: string;
  /** Currently active file in the editor */
  activeFile?: FileType;
  /** Callback when a file is clicked */
  onFileClick: (file: FileType) => void;
  /** Whether AI is thinking about the next phase */
  isThinkingNext?: boolean;
  /** Number of completed files */
  progress: number;
  /** Total number of files to generate */
  total: number;
  /** Reference to parent scroll container */
  parentScrollRef?: RefObject<HTMLDivElement | null>;
  /** Callback to change view to blueprint */
...
function calculateIncrementalLineCount(
  ...
  for (let i = currentPhaseIndex - 1; i >= 0; i--) {
    ...
    const previousPhaseFile = previousPhase.files.find(
    ...
    if (previousPhaseFile && previousPhaseFile.contents) {
      ...
      return Math.max(0, currentTotalLines - previousLines);
    }
  }
...
export function PhaseTimeline({
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        const shouldCollapse = entry.intersectionRatio < 0.7;
        const hasContent =
          projectStages.length > 0 || phaseTimeline.length > 0;
  ...
  const collapsedBarInfo = useMemo(() => {
    ...
    const phaseBadge =
      phaseTimeline.length > 0
        ? `${completedPhases}/${phaseTimeline.length}`
        : undefined;
    ...
    return {
      ...
      subtitle:
        completedStages > 0
          ? `${completedStages}/${projectStages.length} stages complete`
          : "Waiting to begin...",
      icon: (
        <div className="w-4 h-4 bg-gradient-to-br from-muted to-muted-foreground/20 rounded-full" />
      ),
      badge: undefined,
    };
  }, [phaseTimeline, isThinkingNext, progress, total, projectStages]);
  ...
  return (
    <>
      {/* Collapsed Bar with elegant compression animation */}
      <AnimatePresence>
        {showCollapsedBar && (
          <motion.div
            initial={{
          ...
          >
            ...
            <motion.div
              className="absolute inset-0 backdrop-blur-sm rounded-2xl bg-gradient-to-b from-background/50 via-background/25 to-transparent"
              initial={{
                opacity: 0,
                scaleY: 0.8,
                transformOrigin: "top",
                ...
                {isCollapsedBarExpanded && (
                  ...
                  >
                    <div className="p-4 space-y-4">
                      {/* Files List */}
                      {getCurrentPhaseInfo &&
                        getCurrentPhaseInfo.files.length > 0 && (
                          <div className="space-y-1">
                            <div className="text-xs font-medium text-muted-foreground mb-2">
                              Files ({getCurrentPhaseInfo.files.length}):
                            </div>
                            <div className="max-h-32 overflow-y-auto">
                              {getCurrentPhaseInfo.files
                                .slice(0, 5)
                                .map((file, index) => (
                                  <div
                                    key={file.path}
                                    className="relative flex items-center gap-2 text-xs py-1"
                                  >
                                    {index > 0 && (
                                      <div className="absolute left-1.5 -top-1 w-px h-2 bg-primary/60" />
                                    )}
                                    ...
                                    {index <
                                      Math.min(
                                        getCurrentPhaseInfo.files.length,
                                        5
                                      ) -
                                        1 && (
                                      <div className="absolute left-1.5 bottom-0 w-px h-1 bg-primary/60" />
                                    )}
                                  </div>
                                ))}
                              {getCurrentPhaseInfo.files.length > 5 && (
                                <div className="relative flex items-center gap-2 text-xs py-1">
                                  ...
                                  <span className="text-muted-foreground ml-1">
                                    +{getCurrentPhaseInfo.files.length - 5} more
                                    files...
                                  </span>
                                </div>
                              )}
                      ...
                      <div className="flex items-center gap-2 pt-2">
                        <button
                          ...
                          className="flex items-center gap-1.5 px-3 py-1.5 bg-muted hover:bg-muted/80 border border-border rounded-lg text-xs font-medium text-foreground transition-colors"
                        >
        ...
        >
          <div className="border-l-4 border-l-orange-400 pl-4 pr-4 py-4">
            ...
            {(() => {
              ...
              const isAnythingHappening = isGenerating || isThinking;
              const willShowStatusStage =
                allStagesCompleted && !isAnythingHappening;
                          ...
                          <div className="space-y-2 max-h-[400px] overflow-y-auto pr-2 relative">
                            {phaseTimeline.map((phase, phaseIndex) => (
                              ...
                              >
                                ...
                                {phaseIndex < phaseTimeline.length - 1 && (
                                  <div className="absolute left-[5px] w-[0.5px] h-full top-3 bg-border/40" />
                                )}
                                ...
                                <button
                                  ...
                                  className="flex items-start gap-2 relative z-0 w-full text-left hover:bg-muted/50 rounded px-1 py-1 transition-colors group"
                                  disabled={
                                    phase.status !== "completed" &&
                                    phase.status !== "cancelled"
                                  }
                                  ...
                                  <div className="ml-6 space-y-0.5">
                                    {phase.files.map((phaseFile) => {
                                      const globalFile = files.find(
                                        (f) => f.filePath === phaseFile.path
                                      );
                                      const isFileActive =
                                        view === "editor" &&
                                        activeFile?.filePath === phaseFile.path;

                                      return (
                                        <button
                                          key={phaseFile.path}
                                          onClick={() =>
                                            globalFile && onFileClick(globalFile)
                                          }
                                          className="flex items-start gap-2 py-1 transition-colors font-mono w-full text-left group hover:bg-muted/50 rounded px-2 min-h-0"
                            ...
                            {(() => {
                              const validatingPhase = getPhaseByStatus(
                                phaseTimeline,
                                "validating"
                              );
                              if (validatingPhase) {
                                return (
                                  <div className="space-y-1 relative bg-blue-50/5 border border-blue-200/20 rounded-lg p-3">
                                    <div className="flex items-center gap-2">
                                      <StatusLoader  color="blue" />
                                      <span className="text-sm font-medium text-blue-400">
                                        Reviewing phase...
                                      </span>
                                    </div>
                                    <span className="text-xs text-blue-300/80 ml-5">
                                      Identifying issues...
                                    </span>
                                  </div>
                                );
                              ...
                              files.map((file) => {
                                ...
                                return (
                                  <button
                                    ...
                                    className="flex items-start gap-2 py-1 font-mono w-full text-left group hover:bg-muted/50 rounded px-2 min-h-0"
                                  >
                                    <span className="flex-shrink-0">
                                      {file.isGenerating ? (
                                        <StatusLoader  color="accent" />
                                      ) : (
                                        <StatusCheck  color="green" />
                                      )}
                                    </span>
                                    <div className="flex-1 min-w-0">
                                      <span
                      ...
                      if (showDone) {
                        return (
                          ...
                          >
                            ...
                            <div className="absolute left-[9.25px] w-px h-[0.875rem] -top-[0.875rem] bg-primary" />
