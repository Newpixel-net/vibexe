 ...
 */
export async function getAppById(appId: string, userId: string) {
...
export async function getAppsForTeam(teamDbId: number) {
  return db.query.builderApps.findMany({
...
export async function createVersion(appId: string, message: string) {
  ...
  const nextVersionNumber = String((Number(latestVersion?.versionNumber) || 0) + 1);
