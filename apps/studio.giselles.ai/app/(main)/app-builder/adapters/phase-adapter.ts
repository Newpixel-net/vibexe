...
/**
 * AI SDK tool call representation (from useChat hook)
 */
interface ToolCall {
  toolCallId: string;
  toolName: string;
...
/**
 * AI SDK tool result representation
 */
...
export function updatePhaseWithFile(
  ...
  return {
    ...
    description: generatePhaseDescription(toolName, newFiles.map((f) => f.path)),
    files: newFiles,
  };
}
...
export function updatePhaseStatus(
  ...
  return phases.map((phase) => {
    const fileIndex = phase.files.findIndex(
