...
export const config = {
	matcher: [
		"/((?!_next/static|_next/image|.well-known|webhooks|legal|login|signup|join|pricing|password_reset|subscription|auth|preview|api/vector-stores/cron|api/cron|api/apps|robots\.txt|favicon.ico|.*\.(?:svg|png|jpg|jpeg|gif|webp)$).*)",
	],
};
