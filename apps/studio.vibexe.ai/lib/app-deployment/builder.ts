...
_a(){var h=Object.assign({},this.h);var t=typeof window!=="undefined"?localStorage.getItem("vibexe_session"):null;if(t)h["Authorization"]="Bearer "+t;return h}
async list(e,o){o=o||{};var p=new URLSearchParams();if(o.page)p.set("page",String(o.page));if(o.limit)p.set("limit",String(o.limit));if(o.sort)p.set("sort",o.sort);if(o.order)p.set("order",o.order);if(o.filter){for(var k in o.filter){var v=o.filter[k];if(v!==null&&typeof v==="object"&&!Array.isArray(v)){for(var op in v){if(op==="in"&&Array.isArray(v[op]))p.set("filter["+k+"][in]",v[op].join(","));else if(v[op]!==undefined)p.set("filter["+k+"]["+op+"]",String(v[op]))}}else p.set("filter["+k+"]",String(v))}}if(o.search)p.set("search",o.search);if(o.include&&o.include.length)p.set("include",o.include.join(","));var q=p.toString();var r=await fetch(this.b+"/data/"+e+(q?"?"+q:""),{headers:this._a()});if(!r.ok){var err=await r.json().catch(function(){return{}});throw new Error(err.error||"Failed")}return await r.json()}
async aggregate(e,o){o=o||{};var p=new URLSearchParams();if(o.group)p.set("group",o.group);if(o.count)p.set("count","true");if(o.sum)p.set("sum",o.sum);if(o.avg)p.set("avg",o.avg);if(o.min)p.set("min",o.min);if(o.max)p.set("max",o.max);if(o.filter){for(var k in o.filter){var v=o.filter[k];if(v!==null&&typeof v==="object"&&!Array.isArray(v)){for(var op in v){if(op==="in"&&Array.isArray(v[op]))p.set("filter["+k+"][in]",v[op].join(","));else if(v[op]!==undefined)p.set("filter["+k+"]["+op+"]",String(v[op]))}}else p.set("filter["+k+"]",String(v))}}var q=p.toString();var r=await fetch(this.b+"/data/"+e+"/aggregate"+(q?"?"+q:""),{headers:this._a()});if(!r.ok){var err=await r.json().catch(function(){return{}});throw new Error(err.error||"Aggregate failed")}return await r.json()}
async get(e,id,o){var p=new URLSearchParams();if(o&&o.include&&o.include.length)p.set("include",o.include.join(","));var q=p.toString();var r=await fetch(this.b+"/data/"+e+"/"+id+(q?"?"+q:""),{headers:this._a()});if(!r.ok)throw new Error("Not found");return(await r.json()).data}
async create(e,d){var r=await fetch(this.b+"/data/"+e,{method:"POST",headers:Object.assign({},this._a(),{"Content-Type":"application/json"}),body:JSON.stringify(d)});if(!r.ok)throw new Error("Create failed");return(await r.json()).data}
async update(e,id,d){var r=await fetch(this.b+"/data/"+e+"/"+id,{method:"PUT",headers:Object.assign({},this._a(),{"Content-Type":"application/json"}),body:JSON.stringify(d)});if(!r.ok)throw new Error("Update failed");return(await r.json()).data}
async delete(e,id){var r=await fetch(this.b+"/data/"+e+"/"+id,{method:"DELETE",headers:this._a()});if(!r.ok)throw new Error("Delete failed")}
async listRelated(e,id,rel,o){o=o||{};var p=new URLSearchParams();if(o.page)p.set("page",String(o.page));if(o.limit)p.set("limit",String(o.limit));if(o.sort)p.set("sort",o.sort);if(o.order)p.set("order",o.order);if(o.filter){for(var k in o.filter){var v=o.filter[k];if(v!==null&&typeof v==="object"&&!Array.isArray(v)){for(var op in v){if(op==="in"&&Array.isArray(v[op]))p.set("filter["+k+"][in]",v[op].join(","));else if(v[op]!==undefined)p.set("filter["+k+"]["+op+"]",String(v[op]))}}else p.set("filter["+k+"]",String(v))}}var q=p.toString();var r=await fetch(this.b+"/data/"+e+"/"+id+"/"+rel+(q?"?"+q:""),{headers:this._a()});if(!r.ok)throw new Error("List related failed");return await r.json()}
async createRelated(e,id,rel,d){var r=await fetch(this.b+"/data/"+e+"/"+id+"/"+rel,{method:"POST",headers:Object.assign({},this._a(),{"Content-Type":"application/json"}),body:JSON.stringify(d)});if(!r.ok)throw new Error("Create related failed");return(await r.json()).data}
async deleteWithInfo(e,id,o){var p=new URLSearchParams();if(o&&o.dryRun)p.set("dryRun","true");var q=p.toString();var r=await fetch(this.b+"/data/"+e+"/"+id+(q?"?"+q:""),{method:"DELETE",headers:this._a()});if(!r.ok)throw new Error("Delete failed");return await r.json()}
async createMany(e,recs){var r=await fetch(this.b+"/data/"+e+"/batch",{method:"POST",headers:Object.assign({},this._a(),{"Content-Type":"application/json"}),body:JSON.stringify({records:recs})});if(!r.ok)throw new Error("Batch create failed");return await r.json()}
async updateMany(e,ups){var r=await fetch(this.b+"/data/"+e+"/batch",{method:"PUT",headers:Object.assign({},this._a(),{"Content-Type":"application/json"}),body:JSON.stringify({updates:ups})});if(!r.ok)throw new Error("Batch update failed");return await r.json()}
async deleteMany(e,ids){var r=await fetch(this.b+"/data/"+e+"/batch",{method:"DELETE",headers:Object.assign({},this._a(),{"Content-Type":"application/json"}),body:JSON.stringify({ids:ids})});if(!r.ok)throw new Error("Batch delete failed");return await r.json()}
...
class ModulesClient{getInstalled(){return(typeof window!=="undefined"&&window.__VIBEXE_INSTALLED_MODULES__)||[]}isInstalled(id){return this.getInstalled().indexOf(id)!==-1}getConfig(id){var gs=typeof window!=="undefined"&&window.__VIBEXE_GAME_SETTINGS__;if(!gs||!gs.modules||!gs.modules.installed)return null;var m=gs.modules.installed[id];return m&&m.enabled?m.config||{}:null}}
class VibexeApp{constructor(c){this.appId=c.appId;var env=c.environment||(typeof g!=="undefined"&&g.__VIBEXE_ENV__)||null;var base=(c.baseUrl||g.location.protocol+"//vibexe.online")+"/api/apps/"+c.appId;var h={};if(c.apiKey)h["X-Vibexe-Api-Key"]=c.apiKey;this._env=env;this.data=new DataClient(base,h);this.auth=new AuthClient(base,h);this.functions=new FunctionsClient(base,h);this.integrations=new IntegrationsClient(base,h);this.jobs=new JobsClient(base,h);this.storage=new StorageClient(base,h);this.webhooks=new WebhooksClient(base,h);this.modules=new ModulesClient()}}
