...
async get(e,id,o){var p=new URLSearchParams();if(o&&o.include&&o.include.length)p.set("include",o.include.join(","));var q=p.toString();var r=await fetch(this.b+"/data/"+e+"/"+id+(q?"?"+q:""),{headers:this.h});if(!r.ok)throw new Error("Not found");return(await r.json()).data}
...
async listRelated(e,id,rel,o){o=o||{};var p=new URLSearchParams();if(o.page)p.set("page",String(o.page));if(o.limit)p.set("limit",String(o.limit));if(o.sort)p.set("sort",o.sort);if(o.order)p.set("order",o.order);if(o.filter){for(var k in o.filter){var v=o.filter[k];if(v!==null&&typeof v==="object"&&!Array.isArray(v)){for(var op in v){if(op==="in"&&Array.isArray(v[op]))p.set("filter["+k+"][in]",v[op].join(","));else if(v[op]!==undefined)p.set("filter["+k+"]["+op+"]",String(v[op]))}}else p.set("filter["+k+"]",String(v))}}var q=p.toString();var r=await fetch(this.b+"/data/"+e+"/"+id+"/"+rel+(q?"?"+q:""),{headers:this.h});if(!r.ok)throw new Error("List related failed");return await r.json()}
async createRelated(e,id,rel,d){var r=await fetch(this.b+"/data/"+e+"/"+id+"/"+rel,{method:"POST",headers:Object.assign({},this.h,{"Content-Type":"application/json"}),body:JSON.stringify(d)});if(!r.ok)throw new Error("Create related failed");return(await r.json()).data}
async deleteWithInfo(e,id,o){var p=new URLSearchParams();if(o&&o.dryRun)p.set("dryRun","true");var q=p.toString();var r=await fetch(this.b+"/data/"+e+"/"+id+(q?"?"+q:""),{method:"DELETE",headers:this.h});if(!r.ok)throw new Error("Delete failed");return await r.json()}
