...
import { FileId, WorkspaceId } from "@vibexe-ai/protocol";
...
export async function uploadFile(formData: FormData) {
	...
	await vibexe.uploadFile(file, workspaceId, fileId, fileNameRaw);
}
...
export async function addWebPage(
	input: Parameters<typeof vibexe.addWebPage>[0],
) {
	return await vibexe.addWebPage(input);
}
