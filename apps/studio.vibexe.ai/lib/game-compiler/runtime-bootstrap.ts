...
r.push({system:'Player',status:pm?'ok':'missing',details:pm?{name:pm.name||'unknown',y:pm.position?+pm.position.y.toFixed(2):0,hasPhysicsBody:!!pb}:null});
if(pm&&!pb)problems.push({id:'player-no-physics',severity:'error',msg:'Player mesh has NO physics body'});
