  ...
  (async () => {
    ...
    const _startLoop = () => {
      ...
      function animate(time?: number) {
        ...
        } catch (__frameErr: any) { if (W.__vibexe_bundleGen__ === __bundleGen) console.error('[GameLoop] Frame error:', __frameErr?.message || __frameErr?.name || String(__frameErr), __frameErr?.stack?.split('\n')[1] || ''); }
      }
