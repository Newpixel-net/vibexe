...
(function() {
  ...
  function serializeNode(obj) {
    ...
    if (obj.children) {
      for (var i = 0; i < obj.children.length; i++) {
        ...
        // Skip water internal helpers (underside, overlay) but show main water bodies in hierarchy
    ...
    if (obj.isScene || obj.type === "Scene") {
      ...
      if (swaChildren.length > 0) {
        ...
        otherChildren.push({
          ...
          _isMesh: false, _isLight: false, _isGroup: true, _materialColor: null
        });
      }
  ...
  function onCanvasClick(e) {
    ...
    editor.scene.traverse(function(child) {
      if (child.isMesh && child !== boxHelper &&
        ...
        // Skip water internal helpers but allow main water body raycast selection
        if (child.name.indexOf("__water") === 0) return;
        meshes.push(child);
      }
    });
  ...
  function editorLoop() {
    ...
    try {
      ...
      if (e && e.message && (e.message.includes("already initialized") || e.message.includes("usedTimes") || e.message.includes("is not a function"))) return; // suppress WebGPU r172 bugs
