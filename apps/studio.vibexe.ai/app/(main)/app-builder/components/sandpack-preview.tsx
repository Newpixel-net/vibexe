    ...
    function _autoTerrain() {
      ...
      window.postMessage({
        ...
        settings: {
          ...
          terrainSegments: tc.segments || 128,
...
export function SandpackPreview({
	...
	useEffect(() => {
		...
		const saveAllTransforms = async () => {
			...
			// Persist spawned objects to game settings — MERGE into existing file, don't replace
			const spawnedToSave = spawnedObjectsRef.current;
			try {
				// Read current settings file from DB to avoid overwriting other fields
				const settingsFile = currentFiles.find((f) => f.path?.endsWith("__game-settings.json"));
				if (settingsFile?.content) {
					const existingSettings = JSON.parse(settingsFile.content);
					existingSettings.spawnedObjects = spawnedToSave;
					const mergedJson = JSON.stringify(existingSettings, null, 2);
					if (mergedJson !== settingsFile.content) {
						const currentOnFU = onFileUpdateRef.current;
						if (currentOnFU) currentOnFU(settingsFile.id, mergedJson);
						await fetch(`/api/app-builder/apps/${appId}/files`, {
							method: "PUT",
							headers: { "Content-Type": "application/json" },
							body: JSON.stringify({ path: settingsFile.path, content: mergedJson }),
						});
						console.log("[GameEditor] Saved", spawnedToSave.length, "spawned objects (merged into settings)");
					}
				}
			} catch (err) {
			...
			// Game editor messages
				...
				if (iframe?.contentWindow) {
					...
					if (terrainCfg?.enabled && terrainModuleInstalled) {
						...
						setTimeout(() => {
							// Skip if terrain was already generated by AutoTerrain IIFE or previous trigger
							try {
								const _iw = iframe.contentWindow as any;
								if (_iw?.__vibexe_scene__?.getObjectByName?.("__terrain__")) {
									console.log("[GameEditor] Terrain already exists, skipping bridge-load generation");
									// Still send repaint in case textures need applying
									if (terrainCfg.layers?.length) {
										setTimeout(() => {
											iframe.contentWindow?.postMessage({ type: "terrain-painter-repaint", layers: terrainCfg.layers }, "*");
										}, 500);
									}
									return;
								}
							...
							iframe.contentWindow?.postMessage({
								...
								settings: {
									...
									terrainSegments: terrainCfg.segments || 128,
			...
			if (terrainCfg?.enabled && terrainModuleInstalled) {
				...
				terrainTimerRef.current = setTimeout(() => {
					...
					try {
						...
						if (_ew?.__vibexe_scene__?.getObjectByName?.("__terrain__")) {
							console.log("[GameEditor] Terrain already exists, skipping post-exit generation");
							return;
						}
					} catch {}
				...
				{gameEditor.enabled && isGameMode && (
					activeModulePanel === "terrain-painter" ? (
						...
						<SkyWeatherPanel
							...
							onClose={() => setActiveModulePanel(null)}
							settings={gameEditor.gameSettings}
							moduleId={activeModulePanel as string}
							onChange={(skyConfig) => {
								// Live preview only — updates React state + postMessage, NO file write, NO refresh
								const configKey = activeModulePanel === "sky-weather-advanced" ? "skyWeatherAdvanced" : "skyWeather";
								gameEditor.updateGameSettings({ [configKey]: skyConfig });
							}}
							onSave={(skyConfig) => {
								...
								const updatedSettings = { ...gameEditor.gameSettings, [configKey]: skyConfig, modules };
								handleSaveSettings(updatedSettings);
							}}
						/>
