 ...
 *
 * Side panel displayed next to the phone frame in mobile preview mode.
 * Shows preview URL, QR code for real-device testing,
...
export function MobilePublishPanel({ appId }: MobilePublishPanelProps) {
	...
	const qrSvg = useMemo(() => {
		...
		return generateQrSvg(shareUrl, 120);
	}, [shareUrl]);
	...
	return (
		<div className="w-[248px] space-y-3">
			{/* Preview URL */}
			<div className="p-3.5 rounded-2xl bg-white/[0.04] border border-white/[0.08] backdrop-blur-sm space-y-2.5">
				<p className="text-xs font-medium text-white/60">Preview on device</p>
				{shareUrl ? (
					<div className="flex items-center gap-1.5">
						<a
							...
							className="text-[11px] text-violet-400 hover:text-violet-300 truncate flex-1"
							title={shareUrl}
						>
							{displayUrl.length > 28 ? `${displayUrl.slice(0, 25)}...` : displayUrl}
						</a>
						<button
							type="button"
							onClick={handleCopy}
			...
			{/* QR Code */}
			<div className="p-3.5 rounded-2xl bg-white/[0.04] border border-white/[0.08] backdrop-blur-sm">
				<div className="flex flex-col items-center gap-2.5">
					{qrSvg ? (
						<div
							className="rounded-xl overflow-hidden bg-white p-1.5"
							/* biome-ignore lint/security/noDangerouslySetInnerHtml: Generated SVG from internal utility */
							dangerouslySetInnerHTML={{ __html: qrSvg }}
						/>
					) : (
						<div className="w-[132px] h-[132px] rounded-xl bg-white/[0.06] animate-pulse flex items-center justify-center">
							<Smartphone className="w-6 h-6 text-white/20" />
						</div>
					)}
					<p className="text-[11px] text-white/30 leading-relaxed text-center">
						Scan with your phone camera to test on a real device
					</p>
				</div>
			</div>
			...
			<div className="p-3.5 rounded-2xl bg-white/[0.04] border border-white/[0.08] backdrop-blur-sm space-y-2.5">
				<p className="text-xs font-medium text-white/60">Publish to app stores</p>
				<div className="space-y-2">
					{/* Apple App Store */}
					<button
						type="button"
						disabled
						className="w-full flex items-center gap-2 px-3 py-2 rounded-xl bg-white/[0.04] border border-white/[0.06] text-white/30 cursor-not-allowed"
						title="Coming Soon"
					>
						<svg width="16" height="16" viewBox="0 0 18 18" fill="currentColor" className="flex-shrink-0">
						...
						<div className="text-left">
							<div className="text-[8px] leading-none opacity-60">Coming Soon</div>
