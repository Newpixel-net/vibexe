...
import { type DataStoreId, SecretId } from "@vibexe-ai/protocol";
...
/**
 * Updates a data store and its secret in Vibexe storage with rollback support.
 ...
 */
async function updateVibexeDataStoreWithSecret(
	...
	const newSecret = await vibexe.addSecret({
	...
	try {
		...
		await vibexe.deleteSecret({ secretId: newSecret.id }).catch((e) => {
	...
	try {
		await vibexe.deleteSecret({ secretId: oldSecretId });
	} catch (deleteError) {
		try {
			await vibexe.updateDataStore({
				dataStoreId,
			...
			await vibexe.deleteSecret({ secretId: newSecret.id });
		} catch (rollbackError) {
			console.error(
...
/*
 * Creates a secret and data store in Vibexe storage with rollback support.
 * If createDataStore fails, the secret is deleted to prevent orphaned resources.
 */
async function createVibexeDataStoreWithSecret(
	name: string,
	connectionString: string,
): Promise<{ secretId: SecretId; dataStoreId: DataStoreId }> {
	const secret = await vibexe.addSecret({
	...
	try {
		const dataStore = await vibexe.createDataStore({
			provider: "postgres",
			configuration: {
		...
		await vibexe.deleteSecret({ secretId: secret.id }).catch((e) => {
...
export async function createDataStore(
	...
	try {
		const team = await fetchCurrentTeam();
		const { secretId, dataStoreId } = await createVibexeDataStoreWithSecret(
			trimmedName,
		...
		try {
			...
			await Promise.all([
				vibexe.deleteSecret({ secretId }).catch((e) => {
					console.error("Failed to rollback secret:", e);
				}),
				vibexe.deleteDataStore({ dataStoreId }).catch((e) => {
...
export async function updateDataStore(
	...
	try {
		...
		if (connectionString) {
			...
			const existingDataStore = await vibexe.getDataStore({ dataStoreId });
			...
			await updateVibexeDataStoreWithSecret(
				dataStoreId,
				oldSecretId,
...
export async function deleteDataStore(
	...
	try {
		...
		// Get vibexe storage data first (returns undefined if not found)
		const existingDataStore = await vibexe.getDataStore({ dataStoreId });

		// Delete vibexe storage data: secret first (credentials), then dataStore
		if (existingDataStore) {
			const config = parseConfiguration(
				existingDataStore.provider,
				existingDataStore.configuration,
			...
			await vibexe.deleteSecret({ secretId });
			await vibexe.deleteDataStore({ dataStoreId });
		}
