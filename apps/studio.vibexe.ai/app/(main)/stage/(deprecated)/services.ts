...
async function enrichActWithNavigationData(
	...
	try {
		const tmpAct = await vibexe.getTask({ taskId: act.sdkActId });
		const team = teams.find((t) => t.dbId === act.teamDbId);
		if (team === undefined) {
			throw new Error("Team not found");
		}
		const tmpWorkspace = await vibexe.getWorkspace(act.sdkWorkspaceId);
	...
	for (const team of teams) {
		...
		for (const tmpFlowTrigger of tmpFlowTriggers) {
			if (!workspaceMap.has(tmpFlowTrigger.sdkWorkspaceId)) {
				const tmpWorkspace = await vibexe.getWorkspace(
			...
			const flowTrigger = await vibexe.getTrigger({
