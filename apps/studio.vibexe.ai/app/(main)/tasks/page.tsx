...
import { StatusBadge } from "@vibexe-internal/ui/status-badge";
import type { Task, TaskId as TaskIdType } from "@vibexe-ai/protocol";
...
export default async function TaskListPage({
	...
	const tasks = await Promise.all(
		_dbTasks.map(async (dbTask) => {
			const task = await vibexe.getTask({ taskId: dbTask.id });
			if (dbTask.app === null) {
				return {
					id: task.id,
		...
		<div className="px-[40px] py-[24px] flex-1 max-w-[1200px] mx-auto w-full">
			<div className="flex justify-between items-center mb-6">
				...
				<div className="flex items-center gap-4">
					<DocsLink
						href="https://docs.vibexe.ai/en/guides/tasks"
						target="_blank"
