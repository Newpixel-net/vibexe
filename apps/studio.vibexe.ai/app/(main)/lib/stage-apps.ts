import { isTextGenerationNode, isVectorStoreNode } from "@vibexe-ai/protocol";
...
async function getAppsBySampleFlag(
	...
	const result = await Promise.all(
		dbWorkspaces.map(async (dbWorkspace) => {
			let vibexeWorkspace;
			try {
				vibexeWorkspace = await vibexe.getWorkspace(dbWorkspace.id);
			} catch (wsError) {
				logger.warn(
			...
			const appEntryNode = vibexeWorkspace.nodes.find(
			...
			const vibexeApp = await vibexe.getApp({
			...
			for (const connection of vibexeWorkspace.connections ?? []) {
			...
			for (const node of vibexeWorkspace.nodes) {
	...
	for (const data of result) {
		...
		apps.push({
			id: data.vibexeApp.id,
			name: data.workspace.name ?? "Untitled",
			description: data.vibexeApp.description,
			entryNodeId: data.vibexeApp.entryNodeId,
			parameters: data.vibexeApp.parameters,
...
export async function loadStageAppsData() {
	const [currentUser, currentTeam] = await Promise.all([
		getCurrentUser(),
		fetchCurrentTeam(),
	]);
	const [apps, sampleApps] = await Promise.all([
	...
	logger.debug({
		...
		sampleAppsCount: sampleApps.length,
		currentTeamId,
	});
