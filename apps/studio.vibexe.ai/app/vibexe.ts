...
import { traceEmbedding } from "@vibexe-ai/langfuse";
import { getRequestId, NextVibexe } from "@vibexe-ai/nextjs/internal";
import { type RunningGeneration, WorkspaceId } from "@vibexe-ai/protocol";
import type { EmbeddingMetrics } from "@vibexe-ai/rag";
import { fsStorageDriver } from "@vibexe-ai/storage";
import { s3StorageDriver } from "@vibexe-ai/s3-storage-driver";
import { pgVaultDriver } from "@vibexe-ai/vault";
...
// Workspace storage: use S3 only if explicitly opted in via VIBEXE_STORAGE_BACKEND=s3.
// The S3_STORAGE_* env vars are shared with per-app storage (MinIO) but workspace data
// lives on the filesystem by default. Setting VIBEXE_STORAGE_BACKEND=s3 requires
// S3_STORAGE_BUCKET to also be set for the correct bucket name.
const useS3ForWorkspaces =
	process.env.VIBEXE_STORAGE_BACKEND === "s3" &&
	!!(
...
export const storage = useS3ForWorkspaces
	...
	: fsStorageDriver({
			root: process.env.VIBEXE_STORAGE_ROOT || ".storage",
		});
...
const callbacks: VibexeCallbacks = {
	appCreate: async ({ app }) => {
...
export const vibexe = NextVibexe({
	...
	apiSecretScrypt: {
		params: {
			...
			r: parseEnvNumber(process.env.VIBEXE_API_SECRET_SCRYPT_R, 8),
			p: parseEnvNumber(process.env.VIBEXE_API_SECRET_SCRYPT_P, 1),
			keyLen: parseEnvNumber(process.env.VIBEXE_API_SECRET_SCRYPT_KEY_LEN, 32),
		},
		saltBytes: parseEnvNumber(
			process.env.VIBEXE_API_SECRET_SCRYPT_SALT_BYTES,
			16,
		),
		logDuration: process.env.VIBEXE_API_SECRET_SCRYPT_LOG_DURATION === "1",
	},
	onConsumeAgentTime,
...
if (generateContentProcessor === "trigger.dev") {
	vibexe.setGenerateContentProcess(async ({ generation, metadata }) => {
		const requestId = getRequestId();
	...
	vibexe.setRunTaskProcess(async ({ task, generationOriginType }) => {
		const requestId = getRequestId();
