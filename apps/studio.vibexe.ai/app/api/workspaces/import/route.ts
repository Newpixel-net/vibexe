...
export async function POST(request: Request) {
	...
	try {
		...
		// Convert N8N workflow to Vibexe nodes
		// Dynamic import to avoid bundling the adapter in all routes
		const { convertN8NToVibexe } = await import(
			"@vibexe-ai/activepieces-adapter/server"
		);
		const converted = convertN8NToVibexe(
			body.n8nWorkflow as Parameters<typeof convertN8NToVibexe>[0],
		);
		...
		const savedWorkspace = await vibexe.getWorkspace(workspace.id);
		...
		await vibexe.updateWorkspace(savedWorkspace);
