...
describe("POST /api/apps/[appId]/files/upload", () => {
	...
	it("returns 404 when app is not found after auth", async () => {
		...
		// @ts-expect-error - mocked function
		vibexe.getApp.mockResolvedValue(null);
	...
	it("uploads file and returns UploadedFileData", async () => {
		...
		// @ts-expect-error - mocked function
		vibexe.getApp.mockResolvedValue({ workspaceId });
		// @ts-expect-error - mocked function
		vibexe.uploadFile.mockResolvedValue(undefined);
		...
		expect(vibexe.uploadFile).toHaveBeenCalledTimes(1);
		const [
			uploadedFile,
			uploadedWorkspaceId,
			uploadedFileId,
			uploadedFileName,
		] = vi.mocked(vibexe.uploadFile).mock.calls[0];
