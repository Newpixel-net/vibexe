...
export async function POST(
	...
	try {
		...
		const workspace = await vibexe.getWorkspace(workspaceId);
		if (!workspace) {
			return Response.json(
		...
		const stream = new ReadableStream({
			async start(controller) {
				try {
					...
					const taskPromise = vibexe.createAndStartTask({
						workspaceId,
					...
					if (lastGenerationId) {
						const generation =
							await vibexe.getGeneration(lastGenerationId);
						if (
							generation &&
						...
						if (!responseText) {
							const chunks =
								await vibexe.getGenerationMessageChunks({
