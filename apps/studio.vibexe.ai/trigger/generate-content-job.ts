import { GenerationId, isRunningGeneration } from "@vibexe-ai/protocol";
...
export const generateContentJob = schemaJob({
	...
	run: async (payload) => {
		const generation = await vibexe.getGeneration(payload.generationId);
		if (!isRunningGeneration(generation)) {
			return {
				message: `Generation ${payload.generationId} is not running.`,
			};
		}
		await vibexe.generateContent({
			generation,
			logger,
			metadata: {
